...
df_bx_book_ratings = pd.read_csv("../../model/data/BX-Book-Ratings.csv", delimiter=";", encoding="iso-8859-1")
df_bx_book_ratings.columns = ['userID', 'isbn', 'rating']

df_bx_books = pd.read_csv("../../model/data/BX-Books.csv", delimiter=";",encoding="iso-8859-1", error_bad_lines=False)
df_bx_books.columns = ["isbn", "title", "author", "pubyear", "publisher", "img_s", "img_m", "img_l"]

df_bx_book_ratings = df_bx_book_ratings.merge(df_bx_books, on="isbn").dropna()
...
df_top_books = df_ratings.loc[df_ratings['bookID']].isin(top_k_books)
...
def createDataset(train_sparse):
	random.seed(17)
	...
	return train_sparse, dict_altered_samples

train_sparse, dict_altered_samples = createDataset(train_sparse)
